...
def get_keys_list(pattern,tag='irdev'):

    _result=[]
    rInst = init_redis(tag)
    _result = rInst.keys(pattern)
    return _result
def del_keys(key,tag='irdev'):
    _result = False
    rInst = init_redis(tag)
    return rInst.delete(key)
def get_string(key,tag='irdev'):
    _result=None
    rInst = init_redis(tag)
    key = 'string:'+key
    if rInst.exists(key):
    ...
    return _result
def set_string(key,value,tag='irdev'):
    key = 'string:'+key
    rInst=init_redis(tag)
    rInst.set(key,value)
def get_json_to_dict(okey,tag='irdev'):
    _result=None
    rInst = init_redis(tag)
    key = 'json:'+okey
    if rInst.exists(key):
    ...
    return _result
def set_json_from_dict(okey,dict,tag='irdev'):
    key = 'json:'+okey
    rInst=init_redis(tag)
    rInst.set(key,json.dumps(dict))

def get_hash_one(okey,ikey,tag='irdev'):
    rInst=init_redis(tag)
    _hkey = 'hash:'+okey
    _result =0
    ...
    if _result ==None:
        _result =0
    return _result
def get_hash_all(okey,tag='irdev'):
    rInst=init_redis(tag)
    _hkey = 'hash:'+okey
    _result =None
    if rInst.exists(okey):
    ...
    return _result
def del_hash_one(okey,ikey,tag='irdev'):
    rInst=init_redis(tag)
    _result = False
    if rInst.exists(okey):
        _result = rInst.hdel(okey,ikey)
    return _result
def del_hash_all(okey,tag='irdev'):
    rInst=init_redis()
    _hkey = 'hash:'+okey
    _result =False
    if rInst.exists(okey):
...
False - del fail
"""
def set_hash_one(okey,ikey,val,tag='irdev'):
    rInst=init_redis(tag)
    _hkey = 'hash:'+okey
    rInst.hset(okey,ikey,val)
def get_list(key,tag='irdev'):
    _result = []
    rInst = init_redis(tag)
    if rInst.exists('list:'+key):
        ...
        #print _result
    return _result
def set_list(key,val,tag='irdev'):
    rInst = init_redis(tag)
    #rInst.set('list:'+key,val)
    rInst.rpush('list:'+key,val)
def del_list_key(key,tag='irdev'):
    rInst = init_redis(tag)
    if rInst.exists('list:'+key):
