...
def gen_graph_png(graph,file_name,show=True):
    ...
    if file_name.find('.png')==-1:
        w_file_name = file_name+'.png'
    else:
        w_file_name = file_name
    A.draw(w_file_name) # write to file
    print 'success'
def graph_tree_test():
    A=pgv.AGraph(directed=True,strict=True)
    A.add_edge(1,2)
...
def test_find_from_path(graph):
    print '--find from node directly of d--'
    t=find_from_path(graph,'D')
    print t
    print '--find_from_path_all of D--'
